...
- Results table per test with **search + status filter**
...
- Countdown timer that **survives page refresh** (server-computed deadline, not a client-side counter)
- Auto-submits the instant time runs out — even if the browser tab was closed (see [How it works](#how-it-works))
- One-question-at-a-time layout with a question navigator palette
- Confirmation dialog before final submission
- Best-effort deterrents against copy/paste, right-click, text selection, and common DevTools shortcuts
