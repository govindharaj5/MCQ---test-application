...
// NEW (Phase 2, Feature 15): overview now includes the pass/fail split and
// score aggregates computed by GET /dashboard/overview.
function renderOverview(stats) {
  overviewStatsEl.innerHTML = `
    ...
    <div class="stat-card"><div class="stat-label">Passed</div><div class="stat-value success">${stats.totalPassed ?? '—'}</div></div>
    <div class="stat-card"><div class="stat-label">Failed</div><div class="stat-value danger">${stats.totalFailed ?? '—'}</div></div>
    <div class="stat-card"><div class="stat-label">Average Score</div><div class="stat-value">${stats.averageScore ?? '—'}</div></div>
  `;
}
