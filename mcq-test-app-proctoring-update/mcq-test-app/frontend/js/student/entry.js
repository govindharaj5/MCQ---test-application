...
document.getElementById('entryForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // NEW (Phase 2, Feature 14): carry the single-active-session token into
    // the test page via sessionStorage (per-attempt key, never shared between
    // tabs). test.js also re-adopts the token from every session payload.
    if (session.attempt?.sessionToken) {
      sessionStorage.setItem(`mcq_attempt_token_${session.attempt.id}`, session.attempt.sessionToken);
    }
