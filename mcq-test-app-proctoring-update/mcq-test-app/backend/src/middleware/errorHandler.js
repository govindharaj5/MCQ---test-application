...
class ApiError extends Error {
  constructor(statusCode, message, code = null) {
    ...
    this.code = code; // NEW (Phase 2, Feature 14): machine-readable code, e.g. 'session_revoked'
  }
}
...
function errorHandler(err, req, res, next) {
  ...
  res.status(statusCode).json({
    success: false,
    message,
    code: err.code || undefined, // NEW: surfaced to the frontend (api.js attaches it to the error)
  });
}
